...
def q1():
            ...
            with open(pf, "r") as f:
                for l in f:
                    if re.search("^ *from +common +import", l):
                        print(
                            'fichier {}: la syntaxe "from common import XXX" ne doit pas être utilisée'.format(
                                pf
                            )
                        )
                        return False
    ...
    if common.LENGTH != 4:
        print(
            "Attention, common.LENGTH a été modifié, on remet la valeur par défaut (4) pour ces tests"
        )
        common.LENGTH = 4
    if common.COLORS != ["R", "V", "B", "J", "N", "M", "O", "G"]:
        print(
            "Attention, common.COLORS a été modifié, on remet la valeur par défaut pour ces tests"
        )
    ...
    if type(r) is not tuple or len(r) != 2:
        print(
            "q1: common.evaluation('RRVB', 'JORV') ne renvoie pas un tuple de longueur 2"
        )
        return False
    if r != (0, 2):
    ...
    try:
        ...
        print(
            "q1: l'appel à common.evaluation('RVBJN', 'RBVOO') a causé une erreur\n(common.LENGTH avait été mis à 5 pour ce test)"
        )
        traceback.print_exc()
        return False
    common.LENGTH = 4
    if r != (1, 2):
        print(
            "q1: common.evaluation('RVBJN', 'RBVOO') ne renvoie pas la valeur attendue"
        )
        return False
    return True
...
def q5():
    ...
    try:
        ...
        print(
            "q5: l'appel à common.donner_possibles('RRRR', (1, 0)) a causé une erreur"
        )
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    ...
    if q1() and q2():
        print(
            "Auto-tests élémentaires réussis (cela ne garantit pas que vos programmes sont corrects)"
        )
    else:
        print(
            "Échec des auto-tests élémentaires (arrêt à la première erreur rencontrée)"
        )
