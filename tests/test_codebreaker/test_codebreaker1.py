...
from app.mastermind.codebreaker1 import codebreaker, init,tried  # Suppose que ton code est dans `app/mastermind/codebreaker.py`
...
class TestRandomCodeBreaker(unittest.TestCase):
    ...
    def test_guess_format(self):
        ...
        self.assertEqual(len(guess), common.LENGTH, "La proposition doit avoir 4 symboles.")
        self.assertTrue(all(c in common.COLORS for c in guess), f"Couleurs invalides dans {guess}.")
    ...
    def test_no_duplicate_guesses(self):
        ...
        for _ in range(50):  # 50 essais pour détecter une répétition (très improbable)
            ...
            self.assertNotIn(guess, guesses, f"La proposition {guess} a déjà été faite !")
            guesses.add(guess)
    ...
    def test_init_resets_tried_set(self):
        ...
        self.assertIn(first_guess, tried, "La première proposition devrait être dans `tried`.")
    ...
    def test_unused_evaluation_p(self):
        ...
        self.assertNotEqual(guess1, guess2, "Les propositions ne devraient pas dépendre de `evaluation_p`.")
