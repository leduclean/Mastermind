...
def auto_mode(mode, codemaker):
    ...
    play_log(
        codemaker_version=(
            1 if codemaker == "human" else codemaker
        ),  # Met la bonne version de ton codemaker et utilise la version 1 si c'est un humain
...
def human_codemaker():
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("main.game", mode=mode, reset="true", codemaker="human")
        )
    return render_template(
        "human_codemaker.html",
        solution=session.get("solution"),
